...
namespace LibraryManagement.Applicaiton.Handlers.Search.Queries.GetSuggestions
{
    public class GetSuggestionQueryHandler : IRequestHandler<GetSuggestionQuery, PagedData<GetBooksQueryResponse>>
    {
        private readonly ILibraryManagementDbContext db;
        public GetSuggestionQueryHandler(ILibraryManagementDbContext db)
        {
            this.db = db;
        }
        public async Task<PagedData<GetBooksQueryResponse>> Handle(GetSuggestionQuery request, CancellationToken cancellationToken)
        {
            ...
            var result = query.Select(b => new GetBooksQueryResponse()
            {
                ...
                Authors = b.Authors.Where(a => a.DeleteDate == null).Select(a => new GetBooksQueryResponseAuthors
                {
                    Id = a.Id,
